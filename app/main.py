from fastapi import FastAPI, Form, Depends, HTTPException, Request, Header
...
def active_goal_is_hit(goal, log: LogPayload):
    ...
    has_distance_goal = (
        goal["goal_distance"] is not None
    )

    has_duration_goal = (
        goal["goal_duration"] is not None
    )
    ...
    if (
        has_distance_goal
        and log.distance < goal["goal_distance"]
    ):
        return False

    if (
        has_duration_goal
        and log.duration < goal["goal_duration"]
    ):
        return False
...
def complete_goal_if_hit(userid, log, conn):
...
async def create_log(
    ...
    try:
        ...
        if pace > previous["max_pace"]:
            message += (
                f"\nCongratulations! "
                f"You beat your pace PR "
                f"of {previous['max_pace']}!"
            )
        ...
        if log.duration > previous["max_duration"]:
            message += (
                "\nCongratulations! "
                "This is the longest duration yet!"
            )
        ...
        if complete_goal_if_hit(userid, log, conn):
            message += (
                "\nGoal achieved! "
                "Your active goal has been completed."
            )
